...
describe('UIText', () => {
  describe('render', () => {
    ...
    it('should render default styles', () => {
      const { getByTestId } = renderWithTheme(<UIText data-testid="ui-label">field input</UIText>);
  
      expect(getByTestId('ui-label')).toHaveStyle(`font-size: 14px; color: #656565;`);
    });
  });
  describe('custom config', () => {
    test.each([
      ...
      const { getByTestId } = renderWithTheme(
        <UIText data-testid="ui-label" {...{[property]: value}}>field label</UIText>
      );
